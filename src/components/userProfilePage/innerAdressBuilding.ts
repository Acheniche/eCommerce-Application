export default function innerAddres(data:any){
  let i:number = 0
      while(i < data.addresses.length){
      console.log(data.addresses[i].id)
      const container = document.querySelector('#adress-profail-id3');
      let street = data.addresses[i].streetName
      let city = data.addresses[i].city
      let postalCode = data.addresses[i].postalCode
      let country = data.addresses[i].country
...
function adressBilling(typ:string, street:string, city:string, postalCode:string, country:string){
  return `
...
function adressShiping(typ:string, street:string, city:string, postalCode:string, country:string){
    return `
